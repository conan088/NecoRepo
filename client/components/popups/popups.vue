<template>
  <view
    class="shadow"
    :class="!show ? '' : 'shadow-show'"
    :style="{ backgroundColor: show ? maskBg : 'rgba(0,0,0,0)' }"
    @tap="tapMask"
  >
    <view
      class="popups"
    ...
    >
      ...
      <view
        ...
        :class="[
          direction == 'row' ? 'solid-right' : 'solid-bottom',
          item.disabled ? 'disabledColor' : '',
        ]"
      >
        <u-icon size="35" :name="item.icon" v-if="item.icon"></u-icon
