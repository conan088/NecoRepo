      ...
      <view class="goods-box bottom">
        <view class="goods-header">
          ...
          <view class="goods-skus">
            <div class="goods-skus-top">
              ...
              >
                <span v-if="goodsDetail.promotionPrice && !pointDetail">
                  <span class="goods-price-promotionShow goods-price">{{
              ...
              <view>
                <!-- 批发价格 -->
                <div
                  class="price-row flex"
                  v-if="goodsDetail.salesModel === 'WHOLESALE'"
                >
                  <div
                    class="goods-price"
                    v-for="(item, index) in wholesaleList"
                    :key="index"
                  >
                    <span>
                      $
                    ...
                    <span class="wholesale-item"
                      >{{ item.num }}{{ goodsDetail.goodsUnit }}</span
                    >
                  </div>
                </div>
                <div class="goods-price" v-else>
				  ...
				  <span>
				    ...
				    <span class="goods-price">
				      {{Number(goodsDetail.price )}}{{unit}}
				    </span>
				    
				  </span>
                </div>
              </view>
              <view class="goods-check-name">
            ...
            <div class="goods-skus-bom">
              <div class="goods-skus-bom-number">
                <uni-number-box
                  class="uNumber"
                  :min="1"
                  :max="999"
        ...
        <scroll-view class="goods-skus-box" :scroll-y="true">
          ...
          >
            <view class="skus-view-list">
              ...
              <view class="view-class-title">{{ $t("产品类型") }}</view>

              <!-- 正常逻辑 循环出sku -->
              <div class="skus-view-list-box">
                <view
                  v-if="!parentOrder"
                  :class="{
                    active: spec_val.value == currentSelected[specIndex],
                  }"
          ...
          <div class="soldout" v-if="goodsDetail.quantity === 0">
            <u-alert-tips
              type="warning"
              :title="$t('商品已售罄')"
              :description="$t('当前商品库存为0')"
            ></u-alert-tips>
          </div>
          <!-- 数量 -->
          <!-- <view
            v-if="goodsDetail.quantity !== 0"
            class="goods-skus-number flex flex-a-c flex-j-sb"
          >
            <view class="view-class-title">数量</view>
            
          </view> -->
        </scroll-view>
        <div class="goods-skus-total">
          <span>{{ $t("合计") }}：</span>
          <span>{{ toFixedTrunc(totalPrice,3) }}{{unit}}</span>
        </div>
        <!-- 按钮 -->
        <view class="goods_btns" v-if="goodsDetail.quantity !== 0">
          <view
            class="goods_box-btn"
            v-if="
              buyType != 'PINTUAN' && goodsDetail.goodsType != 'VIRTUAL_GOODS'
            "
            @click="addToCartOrBuy('cart')"
            >{{ $t("加入购物车") }}</view
          >
          <view class="goods_box-btn" @click="addToCartOrBuy('buy')">{{
            $t("立即购买")
          }}</view>
        </view>
      </view>
...
export default {
  ...
  data() {
    return {
      setup,
      num:
        this.wholesaleList && this.wholesaleList.length > 0
          ? this.wholesaleList[0].num
          : 1,
    ...
    num: {
      handler(val) {
        ...
        if (val) {
          ...
          this.totalPrice = 
            this.num * (Number(this.goodsDetail.price ) )
