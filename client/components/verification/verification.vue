<template>
  <view>
    <view class="mask" v-if="!hid"></view>
    <view
      v-if="!hid"
      class="flex-row-center"
      :style="{ top: scHight }"
      style="width: 750rpx; position: fixed; z-index: 1000; left: 0"
    >
      <view
      ...
      >
        ...
        <view class="flex-row-between padding-top" style="width: 100%">
          <u-icon
            @click="hide"
            :color="mainColor"
            size="40"
...
export default {
  ...
  data() {
    return {
      ...
      flage: false,
      key: "", //key
      vsrtx: this.$t("点击进行验证"), //按钮提示语
      vsr: false, //
      hid: true,
      col: "#838383",
      movePv: 0,
      hasImg: this.$t("拖动滑块已完成拼图"),
      spcode: "",
      tl: 0,
      moveCode: 0,
      //X轴移动距离
...
getCode(retries = 5) {
  ...
  uni.request({
    ...
    timeout: 8000, // 可选：请求超时时间，避免长时间卡住
    success: (res) => {
      // 这里把“业务失败”也当成失败来重试（根据你的后端返回规范调整）
      const ok = res.statusCode === 200 && res.data && (res.data.code === 0 || res.data.success) && res.data.result;
      if (!ok) {
        if (retries > 0) {
          setTimeout(() => this.getCode(retries - 1), 1000);
        } else {
          uni.showToast({ title: this.$t("获取验证码失败"), icon: "none" });
        }
        return;
      }

      this.col = "#838383";
      this.hasImg = this.$t("拖动滑块以完成拼图");
    ...
    fail: () => {
      if (retries > 0) {
        ...
        // uni.showToast({ title: this.$t("Network Error"), icon: "none" });
      }
    }
  });
},
