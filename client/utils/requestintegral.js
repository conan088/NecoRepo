...
http.interceptors.request.use(
  (config) => {
    config.header.lang = store.state.lang;
    ...
    // 白名单接口无需 token 验证
    const whitelistApi = whiteList.some((item) => {
      return config.url.includes(item);
    });

    if (!whitelistApi && !accessToken) return cleanStorage();
...
http.interceptors.response.use(
  async (response) => {
    ...
    if ((token && response.statusCode === 403) || response.data.code === 403) {
      if (!isRefreshing) {
