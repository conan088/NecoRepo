...
export default {
  ...
  getShow() {
    if (
      ...
      uni.getStorageSync(`show`) === undefined
    ) {
      return true;
    }
  ...
  getImGoodsLink() {
   ...
   manageUserCache(userInfo) {
	   console.log(userInfo)
    let users = uni.getStorageSync('cachedUsers') || []
	console.log(users,'users')
    const isExist = users.some(user => user.nickName === userInfo.nickName)
    // 如果不存在则添加
    if (!isExist && userInfo.nickName) {
      users.push(userInfo)
      // 存储更新后的数组
      uni.setStorageSync('cachedUsers', users)
      return true // 表示新增了用户
    }
    return false // 表示未新增
  }
